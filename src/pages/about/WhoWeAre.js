...
function WhoWeAre() {
  ...
  <>
      <Container fluid>
      {
        aboutData.length && aboutData.map((row, index)=> {
            if(index % 2 === 0){
                return <Row className = "justify-content-center my-5" key={row._id}>
                    ...
                    <Col xxl={4} lg={5} md={6} sm={8} xs={10} className="text-md-start  text-start">
                        <p className="about-second-heading mt-lg-4 mt-md-2">{row.title}</p>
                        <hr className="about-who-first-hr " />
                        <p className="about-second-para">{row.description}</p>
                    </Col>
                </Row>
            } else {
            return <Row className="my-5 justify-content-center" key={row._id}>
           
                <Col xxl={4} lg={5} md={6} sm={8} xs={10} className="text-md-end">
                    <p className="about-second-heading mt-lg-4 mt-md-2">{row.title || 'Title'}</p>
                    <hr className="about-who-first-hr ms-md-auto "/>
                    <p className="about-second-para">{row.description || 'Description'}</p>
                </Col>
