...
function Blogs({ blogList }) {
  ...
  const monthChange = (month, year) => {
    ...
    let newBlogList = blogList.filter((blog) => {
      if (
        new Date(blog.date).getMonth() == month &&
        new Date(blog.date).getFullYear() == year
      ) {
        return true;
      }
    });
              ...
              <div className="blog-calender-2022">
                <Dropdown className="d-inline ">
                  ...
                  <Dropdown.Menu>
                    <Dropdown.Item
                      href="#"
                      className="calender-filter-months"
                      onClick={() => monthChange(3, 2022)}
                    >
                ...
                {filteredBlogList.length > 0
                  ? filteredBlogList.map((blog) => (
                      <Col key={blog._id} xs={12} md={6} className="mt-5">
                        <Card
                          className="h-100 border-0 px-2 cursor-pointer"
                          onClick={() => router.push(`/blogs/${blog._id}`)}
                        >
                          <Card.Img
                            variant="top"
                            src={blog?.images[0]?.img || Honey}
                            alt={blog.title}
                          />
                          <Card.Body className="blog-card-padding">
                            <p className="herbal-blogs-card-date">
                              &minus;&minus; &nbsp;
                              {new Date(blog.date).toDateString(undefined, {
